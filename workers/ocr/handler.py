...
def start_handler(event: dict[str, Any], _context: Any) -> dict[str, Any]:
    ...
    # Delivery outputs live at cases/<id>/delivery/<artifact>-v<n>.pdf (cover memo, etc.) — generated by
    # the API (delivery.ts), NOT uploaded records. Same defect class the /_rendered/ skip fixed: the
    # cases/ EventBridge rule fires on their write, _resolve_document 404s, and (key startswith cases/)
    # start_handler RAISES → exhausts retries → ocr-start DLQ fills with delivery PDFs, holding the
    # runaway alarm RED so it can't warn about a REAL failure (aws-cloud-sme audit 2026-06-17: DLQ at
    # 43, alarm RED since 6/12, most recent = a delivery/cover-memo). Skip them like _rendered/.
    # (Proper long-term fix: write generated outputs OUTSIDE the cases/ prefix so the rule never fires.)
    if "/delivery/" in key:
        print(f"skipping delivery output (not an OCR input): {key}")
        return {"started": [], "skipped": "delivery_output"}
