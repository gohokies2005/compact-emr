// Backend mirror of frontend/src/lib/conditionLabel.ts (formatConditionLabel). Cleans condition
// labels coming from Jotform slugs, the library, and RN free-text so "osa" / "Sleep Apnea" /
// "Sleep Apnea (OSA)" / "unspecified_genitourinary" all render ONE clean way. Used by the cover-memo
// subject line so the memo header matches how the nexus letter / EMR UI renders the condition
// (E4 bug fix 2026-06-14: the memo header was emitting the raw lowercased "osa").
//
// Kept byte-aligned with the frontend formatter (same ACRONYMS + CANON tables, same algorithm) so
// the EMR UI and the rendered memo never disagree on a condition's display label.
...
/**
 * Clean, consistent display label for a condition. Mirrors the frontend formatter exactly:
 *   "osa" → "Obstructive Sleep Apnea (OSA)", "ptsd" → "PTSD",
 *   "unspecified_genitourinary" → "Unspecified Genitourinary".
 * Empty/null in → "" out.
 */
