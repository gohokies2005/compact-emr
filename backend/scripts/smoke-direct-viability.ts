...
const cases: Case[] = [
  { n: 1, hard: true, label: 'witnessed combat death -> PTSD (ANCHOR)', claimed: 'PTSD', want: ['viable'],
    facts: { currentDxPresent: true, dxConstellation: ['PTSD', 'Major depressive disorder'], inServiceEvents: EV('witnessed combat death', 'saw squadmate killed by IED in Iraq'), continuityEvidence: null, upstreamScIfAny: null, veteranStatement: 'I watched my friend get killed by the blast and cannot stop reliving it' } },
  { n: 2, hard: true, label: 'ankle sprain -> diabetes, DIRECT (ANCHOR)', claimed: 'Type 2 Diabetes Mellitus', want: ['not_viable'],
    facts: { currentDxPresent: true, dxConstellation: ['Type 2 Diabetes Mellitus', 'Hyperlipidemia'], inServiceEvents: EV('right ankle sprain', 'twisted ankle on a training run, treated at sick call'), continuityEvidence: null, upstreamScIfAny: null, veteranStatement: 'my diabetes started because of the ankle injury I got in the service' } },
  { n: 3, label: 'acoustic trauma -> tinnitus', claimed: 'Tinnitus', want: ['viable'],
    facts: { currentDxPresent: true, dxConstellation: ['Tinnitus', 'Sensorineural hearing loss'], inServiceEvents: EV('acoustic trauma', 'artillery crew, routine unprotected exposure to howitzer fire'), continuityEvidence: null, upstreamScIfAny: null, veteranStatement: 'my ears have rung constantly since the gun line' } },
  { n: 4, label: 'documented knee injury -> same-knee OA', claimed: 'Right Knee Osteoarthritis', want: ['viable'],
    facts: { currentDxPresent: true, dxConstellation: ['Right knee osteoarthritis'], inServiceEvents: EV('right knee injury', 'documented right knee injury at sick call and on the MEB'), continuityEvidence: null, upstreamScIfAny: null, veteranStatement: 'I hurt my right knee in a fall during a field exercise' } },
  { n: 5, label: 'back strain + documented continuity -> lumbar DDD', claimed: 'Lumbar Degenerative Disc Disease', want: ['viable', 'borderline'],
    facts: { currentDxPresent: true, dxConstellation: ['Lumbar degenerative disc disease'], inServiceEvents: EV('low back strain', 'in-service lumbar strain treated in the field'), continuityEvidence: 'STRs and post-service records document continuous low back pain from separation to the present with no symptom-free gap', upstreamScIfAny: null, veteranStatement: 'my back has hurt every day since I strained it in service' } },
  { n: 6, label: 'back strain, NO documented continuity -> borderline', claimed: 'Lumbar Degenerative Disc Disease', want: ['borderline'],
    facts: { currentDxPresent: true, dxConstellation: ['Lumbar degenerative disc disease'], inServiceEvents: EV('low back strain', 'single in-service lumbar strain, then no back complaints for years'), continuityEvidence: null, upstreamScIfAny: null, veteranStatement: 'my back hurts now' } },
  { n: 7, label: 'PTSD claimed, NO current dx -> records-gap borderline', claimed: 'PTSD', want: ['borderline'],
    facts: { currentDxPresent: false, dxConstellation: [], inServiceEvents: EV('claimed stressor', 'veteran describes a stressor but it is unverified and there is no PTSD diagnosis of record'), continuityEvidence: null, upstreamScIfAny: null, veteranStatement: 'bad things happened over there and I have not been the same' } },
  { n: 8, label: 'MST with markers -> PTSD', claimed: 'PTSD', want: ['viable'],
    facts: { currentDxPresent: true, dxConstellation: ['PTSD'], inServiceEvents: EV('military sexual trauma', 'MST with corroborating markers per 38 CFR 3.304(f)(5): a documented request for transfer and a drop in performance'), continuityEvidence: null, upstreamScIfAny: null, veteranStatement: 'I was assaulted during my service and asked to transfer right after' } },
  { n: 9, label: 'burn-pit -> constrictive bronchiolitis (RIGHT organ)', claimed: 'Constrictive Bronchiolitis', want: ['viable'],
    facts: { currentDxPresent: true, dxConstellation: ['Constrictive bronchiolitis'], inServiceEvents: EV('burn-pit / airborne-hazard exposure', 'documented daily proximity to open burn pits at a forward base'), continuityEvidence: null, upstreamScIfAny: null, veteranStatement: 'I breathed burn pit smoke every day and now I cannot catch my breath' } },
  { n: 10, label: 'OSA "from my SC PTSD", DIRECT -> redirect borderline', claimed: 'Obstructive Sleep Apnea', want: ['borderline'],
    facts: { currentDxPresent: true, dxConstellation: ['Obstructive sleep apnea', 'PTSD'], inServiceEvents: [], continuityEvidence: null, upstreamScIfAny: 'PTSD', veteranStatement: 'my sleep apnea is caused by my service-connected PTSD' } },
];
